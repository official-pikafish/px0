...
InputsOutputs::InputsOutputs(OnnxNetwork* network)
  ...
  switch (provider_) {
    ...
    case OnnxProvider::TRT:
      ...
      ReportCUDAErrors(
          cudaMalloc(&input_tensor_data_device_,
                     max_batch_size * kInputPlanes * 10 * 9 * data_size));
      for (int i = 0; i < outputs_size; i++) {
        ReportCUDAErrors(
    ...
    default:
      input_tensor_data_ =
          malloc(max_batch_size * kInputPlanes * 10 * 9 * data_size);
      for (int i = 0; i < outputs_size; i++) {
        output_tensors_data_[i] =
...
Ort::IoBinding OnnxComputation<DataType>::PrepareInputs(int start,
...
#endif
  {
    ...
    iter += start * kInputPlanes * 10 * 9;
    std::memset(iter, 0, batch_size * kInputPlanes * 10 * 9 * sizeof(DataType));
  ...
  binding.BindInput(
      ...
      Ort::Value::CreateTensor<DataType>(
          ...
          static_cast<DataType*>(inputs_outputs_->input_tensor_data_device_) +
              start * kInputPlanes * 10 * 9,
          batch_size * kInputPlanes * 10 * 9, dims, 4));
  return binding;
}
    ...
    case OnnxProvider::TRT: {
      ...
      if (batch_size < 0) {
        trt_options["trt_profile_min_shapes"] =
            inputs_[0] + ":" + std::to_string(min_batch_size_) + "x124x10x9";
        trt_options["trt_profile_max_shapes"] =
            inputs_[0] + ":" + std::to_string(max_batch_size_) + "x124x10x9";
        trt_options["trt_profile_opt_shapes"] =
            inputs_[0] + ":" + std::to_string(max_batch_size_ / 4) + "x124x10x9";
      } else {
        trt_options["trt_profile_min_shapes"] =
            inputs_[0] + ":" + std::to_string(batch_size - batch_size_ + 1) +
            "x124x10x9";
        trt_options["trt_profile_max_shapes"] =
            inputs_[0] + ":" + std::to_string(batch_size) + "x124x10x9";
        trt_options["trt_profile_opt_shapes"] =
            inputs_[0] + ":" + std::to_string(batch_size) + "x124x10x9";
      }
