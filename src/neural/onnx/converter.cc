...
std::string Converter::MakeSmolgen(OnnxBuilder* builder,
  ...
  flow = builder->Reshape(
      ...
      builder->AddInitializer(
          ...
          Int64OnnxConst({-1, 90 * smolgen_hidden_channels}, {2})));
  flow = builder->MatMul(
      name + "/smolgen/dense1/w", flow,
      *GetWeghtsConverter(layer.mha.smolgen.dense1_w,
                          {90 * smolgen_hidden_channels, smolgen_hidden_sz},
                          {1, 0}));
  flow = builder->Add(
  ...
  flow = builder->Reshape(
      ...
      builder->AddInitializer("/const" + name + "/smolgen/out/shape",
                              Int64OnnxConst({-1, heads, 90, 90}, {4})));
  return flow;
}
...
std::string Converter::AttentionBodyDenseEmbedding(
  ...
  pos_info = builder->Reshape(
      ...
      builder->AddInitializer(
          ...
          Int64OnnxConst({-1, 90, embedding_dense_size}, {3})));
  ...
  flow = builder->Reshape(
      ...
      builder->AddInitializer(
          ...
          Int64OnnxConst({-1, kInputPlanes + embedding_dense_size}, {2})));
...
std::string Converter::MakeAttentionBody(OnnxBuilder* builder,
  ...
  if (NumResBlocks() > 0) {
    ...
    fist_stage_out_C = kInputPlanes;
  } else if (input_embedding == network_format::INPUT_EMBEDDING_PE_DENSE) {
    int embedding_dense_size = weights.ip_emb_preproc_b.size() / 90;
    flow = AttentionBodyDenseEmbedding(builder, flow, weights,
                                       embedding_dense_size);
    fist_stage_out_C = kInputPlanes + embedding_dense_size;
  } else {
...
void Converter::MakePolicyHead(pblczero::OnnxModel* onnx, OnnxBuilder* builder,
  ...
  if (src_.format().network_format().policy() ==
    ...
    auto flow = MakeConvBlock(builder, head.policy1, NumFilters(), NumFilters(),
                              input, "/policy/conv1");
    flow = MakeConvBlock(builder, head.policy, NumFilters(), 52, flow,
    ...
    flow = builder->MatMul(
        ...
        *GetWeghtsConverter(head.ip_pol_w, {pol_channels * 10 * 9, 2062},
                            {1, 0}));
    auto output = builder->Add(options_.output_policy_head, flow,
                               *GetWeghtsConverter(head.ip_pol_b, {2062}));
    builder->AddOutput(output, {options_.batch_size, 2062}, GetDataType());
    onnx->set_output_policy(output);
  }
}
...
void Converter::MakeValueHead(pblczero::OnnxModel* onnx, OnnxBuilder* builder,
  ...
  flow =
      builder->Reshape("/value/reshape", flow,
                       builder->AddInitializer(
                           "/const/value_shape",
                           Int64OnnxConst({-1, val_channels * 10 * 9}, {2})));
  flow = builder->MatMul(
      "/value/dense1/matmul", flow,
      *GetWeghtsConverter(head.ip1_val_w, {val_channels * 10 * 9, 128},
                          {1, 0}));
