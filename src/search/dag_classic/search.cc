...
std::vector<std::string> Search::GetVerboseStats(
  ...
  for (const auto& edge_tuple : edges) {
    ...
    print_head(&oss, edge.GetMove(is_black_to_move).ToString(),
...
void Search::SendMovesStats() const REQUIRES(counters_mutex_) {
  ...
  for (auto& edge : root_node_->Edges()) {
    ...
    if (edge.HasNode()) {
      LOGFILE << "--- Opponent moves after: " << final_bestmove_.ToString();
...
std::pair<int, int> SearchWorker::GetRepetitions(int depth,
  ...
  const auto plies = history.Last().GetPliesSincePrevRepetition();
  if (params_.GetTwoFoldDraws() && /*repetitions == 1 &&*/ depth >= 4 &&
    ...
    if (result == GameResult::DRAW) {
      ...
      if (idx2 > 0 && history.GetPositionAt(idx - 1).GetBoard() ==
        ...
        --idx;
        while (++idx2 != idx && !history.GetPositionAt(idx2).GetRepetitions());
    ...
    if (current_path.back() == -1) {
      ...
      while (cur_limit > 0) {
        ...
        auto [child_repetitions, child_moves_left] =
            GetRepetitions(full_path.size(), history);
